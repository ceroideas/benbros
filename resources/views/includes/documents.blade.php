...
@forelse(App\Models\Document::where('type',$type)->orderBy('order','asc')->get() as $key => $value)
	@php
		$ext = array_reverse(explode('.',$value->document))[0];
	@endphp
	<tr class="text-center document-row" data-name="{{$value->document}}" onclick="window.open('{{url('uploads/documents',$value->document)}}','_blank')" style="cursor: pointer;">
			
		<td>
			<img src="{{url('file-images/'.$ext.'.png')}}" onerror="this.src = '{{url('file-images/file.png')}}'" style="width: 30px;" alt="">
		</td>
		<td title="{{ substr($value->document, 14) }}">
			<span style="
			text-overflow: ellipsis;
		    white-space: nowrap;
		    overflow: hidden;
			position: relative; top: 4px; display: block; width: 100%; text-align: left; font-size: 13px">{{ substr($value->document, 14) }} {{ strlen($value->document) > 54 ? '' : '' }} </span>
		</td>
		...
		<td>
			@php
				$fz = filesize(public_path().('/uploads/documents/'.$value->document));
			@endphp
				...
				<div class="modal-dialog modal-sm">
					<div class="modal-content">
						...
						<div class="modal-footer">
							<a class="btn btn-sm btn-success" href="{{url('delete-document',$value->id)}}">{{trans('documents.yes')}}</a>
...
{{-- <div class="row">
@forelse(App\Models\Document::where('type',$type)->orderBy('order','asc')->get() as $key => $value)
	<div class="col-6 text-center document-row" data-name="{{$value->document}}" onclick="window.open('{{url('uploads/documents',$value->document)}}','_blank')" style="cursor: pointer;">

		<div class="row">
			
			<div class="col-2">
				<img src="{{url('file-images/file.png')}}" style="width: 30px;" alt="">
			</div>

			<div class="col-10" title="{{ substr($value->document, 14) }}">
				<span style="
				text-overflow: ellipsis;
			    white-space: nowrap;
			    overflow: hidden;
