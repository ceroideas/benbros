...
@forelse(App\Models\AdministrationDocument::where('administration_id',$a->id)->get() as $key => $value)
	@php
		$ext = array_reverse(explode('.',$value->url))[0];
	@endphp
	<tr class="text-center document-row" data-name="{{$value->url}}" style="cursor: pointer;">
			
		<td onclick="window.open('{{url('uploads/administration-documents',$value->url)}}','_blank')">
			<img src="{{url('file-images/'.$ext.'.png')}}" onerror="this.src = '{{url('file-images/file.png')}}'" style="width: 30px;" alt="">
		</td>
		<td title="{{ substr($value->url, 14) }}" onclick="window.open('{{url('uploads/administration-documents',$value->url)}}','_blank')">
			<span style="
		...
		<td>
			@php
				$fz = filesize(public_path().('/uploads/administration-documents/'.$value->url));
			@endphp
				...
				<div class="modal-dialog modal-sm">
					<div class="modal-content">
						...
						<div class="modal-footer">
							<a class="btn btn-sm btn-success" href="{{url('delete-administrative-document',$value->id)}}">{{trans('documents.yes')}}</a>
...
@forelse(App\Models\Document::where('type',$type)->orderBy('order','asc')->get() as $key => $value)
	<div class="col-6 text-center document-row" data-name="{{$value->url}}" onclick="window.open('{{url('uploads/documents',$value->url)}}','_blank')" style="cursor: pointer;">
		...
		<div class="row">
			...
			<div class="col-10" title="{{ substr($value->url, 14) }}">
				<span style="
				text-overflow: ellipsis;
			    white-space: nowrap;
			    overflow: hidden;
