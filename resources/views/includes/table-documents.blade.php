...
@forelse($documents as $key => $value)
	...
	<tr class="text-center document-row" data-name="{{$value->url}}" style="cursor: pointer;">
		...
		<td onclick="window.open('{{url('uploads/documents/permissions',$value->url)}}','_blank')">
			<img src="{{url('file-images/'.$ext.'.png')}}" onerror="this.src = '{{url('file-images/file.png')}}'" style="width: 30px;" alt="">
		</td>
		<td title="{{ substr($value->url, 0) }}" onclick="window.open('{{url('uploads/documents/permissions',$value->url)}}','_blank')">
			<span style="
			text-overflow: ellipsis;
		    white-space: nowrap;
		    overflow: hidden;
		    font-weight: normal !important;
			position: relative; top: 4px; display: block; width: 100%; text-align: left; font-size: 13px">{{ substr($value->url, 0,40) }} {{ strlen($value->url) > 54 ? '...' : '' }} </span>
		</td>
		...
		<td style="font-weight: normal !important;">
			@php
				$fz = filesize(public_path().('/uploads/documents/permissions/'.$value->url));
			@endphp
				...
				<div class="modal-dialog modal-sm">
					<div class="modal-content">
						...
						<div class="modal-footer">
							<a class="btn btn-sm btn-success" href="{{url('delete-permission-document',$value->id)}}">{{trans('documents.yes')}}</a>
