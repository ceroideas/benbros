...
@forelse(App\Models\OtherInformation::orderBy('id','asc')->get() as $key => $value)
	@php
		$ext = array_reverse(explode('.',$value->url))[0];
	@endphp
	<tr class="text-center document-row" data-name="{{$value->url}}" style="cursor: pointer;">
			
		<td onclick="window.open('{{url('uploads/other-information',$value->url)}}','_blank')">
			<img src="{{url('file-images/'.$ext.'.png')}}" onerror="this.src = '{{url('file-images/file.png')}}'" style="width: 30px;" alt="">
		</td>
		<td title="{{ substr($value->url, 14) }}" onclick="window.open('{{url('uploads/other-information',$value->url)}}','_blank')">
			<span style="
		...
		<td>
			@php
				$fz = filesize(public_path().('/uploads/other-information/'.$value->url));
			@endphp
				...
				<div class="modal-dialog modal-sm">
					<div class="modal-content">
						...
						<div class="modal-footer">
							<a class="btn btn-sm btn-success" href="{{url('delete-other-information',$value->id)}}">{{trans('documents.yes')}}</a>
