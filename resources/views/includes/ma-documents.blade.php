...
@forelse(App\Models\MaDocument::orderBy('id','asc')->get() as $key => $value)
	@php
		$ext = array_reverse(explode('.',$value->url))[0];
	@endphp
	<tr class="text-center document-row" data-name="{{$value->url}}" style="cursor: pointer;">
			
		<td onclick="window.open('{{url('uploads/ma-documents',$value->url)}}','_blank')">
			<img src="{{url('file-images/'.$ext.'.png')}}" onerror="this.src = '{{url('file-images/file.png')}}'" style="width: 30px;" alt="">
		</td>
		<td title="{{ substr($value->url, 14) }}" onclick="window.open('{{url('uploads/ma-documents',$value->url)}}','_blank')">
			<span style="
		...
		<td>
			@php
				$fz = filesize(public_path().('/uploads/ma-documents/'.$value->url));
			@endphp
